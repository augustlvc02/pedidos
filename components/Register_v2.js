...
export default class Register extends React.Component {
  ...
  ListarProveedor = async() =>{
    try{
      const Url = "https://project-code-dev.herokuapp.com/api/v1/proveedor";
  ...
  ListarProducto = async() =>{
    try{
      const Url = "https://project-code-dev.herokuapp.com/api/v1/producto";
  ...
  GuardarDetallePedido = () =>{
    ...
    if(isEdit && pedidodetalles[position].producto.productoid == producto.productoid) repetidoValid=true;
    ...
    if(!repetidoValid){
      ...
      for (const peddet in pedidodetalles) {
        ...
        if(pedidodetalles[peddet].producto.productoid == producto.productoid) repetido=true;
      }
    }

    if( producto.productoid==0 || pedidodetalle.cantidad==0 || cantidadLenght==0 || repetido)
    {
      if(producto.productoid==0) mensaje='Producto no puede estar vacio';
      ...
      <Container>
        ...
        <Content padder contentContainerStyle={{flex:1}}>
          ...
          <List>
            ...
            <ListItem key={item.producto.productoid} onPress={() => this.EditarProductoList(item)}> 
              <Body>
                <Text>Nombre: {item.producto.nombre_producto}</Text>
                <Text note >Cantidad: {item.cantidad}</Text>
                <Text note>Id: {item.producto.productoid}</Text>
              </Body>
              <Right>
